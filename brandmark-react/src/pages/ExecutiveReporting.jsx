...
export const ExecutiveReporting = () => {
  ...
  const mockReports = [
    { id: 1, title: 'Monthly Revenue Summary', format: 'PDF', schedule: '1st of month', status: 'active' },
    { id: 2, title: 'Weekly Churn Analysis', format: 'Excel', schedule: 'Mondays', status: 'active' },
    { id: 3, title: 'Q3 Board Deck', format: 'PDF', schedule: 'End of Quarter', status: 'paused' }
  ];
                ...
                {mockReports.map(rep => (
                  <tr key={rep.id} className="hover:bg-gray-50 transition-colors">
                    <td className="p-4 font-bold text-gray-700 text-sm flex items-center gap-3">
                       <FileText className="w-5 h-5 text-gray-400"/> {rep.title}
                    </td>
                    ...
                    <td className="p-4 text-sm font-medium text-gray-700 flex items-center gap-2"><Calendar className="w-4 h-4 text-gray-400"/> {rep.schedule}</td>
                    <td className="p-4">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold ${
                        rep.status === 'active' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'
                      }`}>
                        {rep.status === 'active' ? <CheckCircle className="w-3 h-3"/> : null}
